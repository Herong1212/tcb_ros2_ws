...
class RobotArmFunction:
    ...
    def _monitor_movement(self, robotarm6001, target_pos, coord, timeout=60):
        ...
        while not self.stop_flag:
            ...
            # print(f"当前坐标: {current_pos}, 目标坐标: {target_pos}")
        ...
        if servo_status == 3:
            ...
            if current_speed != speed:
                ...
                while servo_status!=3:
                    ...
                    robotarm6001.deadman_status_set(0)
                    robotarm6001.deadman_status_set(1)
                    time.sleep(0.5)
                    # 伺服状态查询
                    servo_status = robotarm6001.servo_status_inquire()
                    if time.time() - start_time > timeout:
    ...
    def robot_init_repeat(self, robotarm6001, speed=35):
        ...
        if servo_status == 3:
            self.logger.info("正在初始化...")
            # 确保打开的作业文件停止运行
            if robotarm6001.job_open_inquire() != "NULL":
    ...
    def robot_encoder_error_clear(self, robotarm6001, servoNum: int=0):
        """
        用于清除伺服错误

        Args:
            robotarm6001(socket.socket): 6001端口连接的socket对象
            servoNum(int): 需要清除错误的关节
        
        Returns:    
            None
        """
        self.logger.info(f'清除伺服错误(robot_encoder_error_clear)')
        if servoNum == 0:
            for joint in range(1, 8):
                error_code, _, _, _, _ =robotarm6001.servo_inside_parm_inqure(joint)
                if error_code == 29443: # 0x7303错误代码
                    # 写6写7然后再清错
                    robotarm6001.servo_inside_parm_set(joint, "编码器命令", 6)
                    robotarm6001.servo_inside_parm_set(joint, "编码器命令", 7)
                    robotarm6001.fault_reset()
        elif 1 <= servoNum <= 7:
            robotarm6001.servo_inside_parm_set(servoNum, "编码器命令", 6)
            robotarm6001.servo_inside_parm_set(servoNum, "编码器命令", 7)
            robotarm6001.fault_reset()
        else:
            self.logger.warning(f'关节号为1-7的整数。0表示全部清错。')
            raise ValueError(f'关节号为1-7的整数。0表示全部清错。')
        
        self.logger.info(f'已经发送伺服清错指令')

    def robot_move(self, robotarm6001, joint_data, movj_speed=100, movs_speed=300):
        """
        控制机械臂执行运动，并检测是否到达目标位置
        主要采用movj、movs控制机械臂关节空间下的移动

        Args:
            robotarm6001(socket.socket): 6001端口连接的socket对象
            joint_data(List[float]、List[List[float]]): 一组关节角度或者轨迹
            movj_speed(int): movj中的速度值
            movs_speed(int): movs中的速度值
            coord(int): 坐标模式
                - 0: 关节坐标(Joint)
                - 1: 直角坐标(Cart)
                - 2: 工具坐标(Tool)
        ...
        self.logger.info(f'机械臂运动(robot_move)')
        timeout = 30  # 45 秒超时
        status, data_len, joint_data_type = self._validate_data(joint_data)
        if not status:
            self.logger.error("检查一下输入的关节数据是否为列表或者双列表,如果是列表(子列表),看看(子列表)是否是7个关节角度")
            return False

        if joint_data_type == "List[float]":
            # 单点移动
            target_pos = joint_data
            ...
            if reached:
                ...
                robotarm6001.movj(movj_speed, 0, joint_data)  # movj的速度设置最高100，这个是全局速度的百分比 

                monitor_thread = threading.Thread(target=self._monitor_movement, args=(robotarm6001, target_pos, 0, timeout))
            ...
            # 检查子列表是否相同
            rounded_joint_data = [list(map(lambda x: round(x, 2), joint)) for joint in joint_data]
            unique_positions = []
            
            for joint in rounded_joint_data:
                if joint not in unique_positions:
                    unique_positions.append(joint)
            
            if len(unique_positions) == 1:
                # 如果所有子列表相同，则调用一次movj
                target_pos = unique_positions[0]
                current_pos = robotarm6001.currentpos_inquiry(0)
                reached = self._check_target_reached(current_pos, target_pos)
                if reached:
                    self.logger.info("机械臂已到达目标点")
                    ...
                    robotarm6001.movj(movj_speed, 0, target_pos)
                    monitor_thread = threading.Thread(target=self._monitor_movement, args=(robotarm6001, target_pos, 0, timeout))
                    monitor_thread.start()
                    monitor_thread.join()
            ...
            elif data_len < 4: # 因为movs至少要4个点位
                # 逐点移动
                for index, target_pos in enumerate(unique_positions):
                    current_pos = robotarm6001.currentpos_inquiry(0)
                    reached = self._check_target_reached(current_pos, target_pos)
                    if reached:
                        self.logger.info("机械臂已到达目标点")
                ...
                robotarm6001.movs(movs_speed, 0, len(unique_positions), unique_positions) # movs这里可以将速度设置到150， 含义与movj不一样
                target_pos = unique_positions[-1]  # 检查最后一个目标点， 判断轨迹是否结束
                time.sleep(0.5) # 这里给延迟是为了排除轨迹第一个点的判断干扰
    ...
    def robot_direction_move(self, robotarm6001, axis, distance, frame="base"):
        ...
        self.logger.info(f'机械臂末端沿着 {axis} 在 {frame} 下直线移动 {distance} m')
        try:
            traj = self.kinematics.straightline(T_EndToBase, current_pos)
            # status = self.robot_move(robotarm6001, traj)
            self.logger.info(f'规划的轨迹为: {traj}')
            return traj
        except Exception as e:
            self.logger.error(f"机械臂门型运动规划失败: {e}")
            return []

    def robot_servo_move(self, robotarm6001, robotarm7000, target_angles, 
                         step_size=0.1, interval_time=0.001):
        """
        伺服点位运动控制(透传、遥操作)
        ...
        Args:
            ...
            target_angles(List[float]、List[List[float]]): 一组关节角度或者轨迹
            step_size(float): 最大步长，决定每轮迭代中最大关节的移动量
            interval_time(float): 每次迭代之间的延迟时间，单位：秒(不能低于控制器的通讯周期, 
            控制器的通讯周期为1、2、4、8ms四个选择情况)
        ...
        Returns:
            bool: 运动状态
                ...
                - False: 运动故障，超时  
        """
        self.logger.info(f'机械臂伺服点位运动控制(robot_servo_move)')
        robotarm7000.set_servo_point_motion_control(True)
        # self.logger.info(f"开始移动指令，数据类型: {type(target_angles)}")
        status, data_len, joint_data_type = self._validate_data(target_angles)
        if not status:
            self.logger.error("检查一下输入的关节数据是否为列表或者双列表, 如果是列表(双列表),看看(子列表)是否是7个关节角度")
            return False
        
        if joint_data_type == "List[float]":
            current_angles = [round(angle, 4) for angle in robotarm6001.currentpos_inquiry(0)]

            delta = [t - c for t, c in zip(target_angles, current_angles)]
            
            # 计算实际需要的迭代次数 (向上取整)
            max_delta = max(abs(d) for d in delta)
            if max_delta <= 1e-3:  # 忽略微小移动
                self.logger.info("机械臂已在目标位置")
                return
            
            # if max(abs(t - c) for t, c in zip(target_angles, current_angles)) < 0.05:
            #     return  # 已在容差范围内不移动
            
            iterations = max(1, math.ceil(max_delta / step_size))
            self.logger.info(f'计算迭代次数: {iterations}')
            
            # 计算每关节实际步长
            step_per_joint = [d / iterations for d in delta]
            
            # 分步移动
            for _ in range(iterations):
                current_angles = [c + s for c, s in zip(current_angles, step_per_joint)]
                robotarm7000.servo_point_motion_control(robot=1, end=0, sum=1, count=1, PosVec=[current_angles])
                time.sleep(interval_time)
            
            # 终点精确补偿 (关键修正)
            robotarm7000.servo_point_motion_control(robot=1, end=0, sum=1, count=1, PosVec=[target_angles])
            time.sleep(interval_time)

        elif joint_data_type == "List[List[float]]":
            for every_joint_angle in target_angles:
                current_angles = [round(angle, 4) for angle in robotarm6001.currentpos_inquiry(0)]

                delta = [t - c for t, c in zip(every_joint_angle, current_angles)]
                
                max_delta = max(abs(d) for d in delta)
                if max_delta <= 1e-3:  # 忽略微小移动
                    self.logger.info("机械臂已在目标位置")
                    return
                # if max(abs(t - c) for t, c in zip(every_joint_angle, current_angles)) < 0.05:
                #     return  # 已在容差范围内不移动
                iterations = max(1, math.ceil(max_delta / step_size))
                self.logger.info(f'计算迭代次数: {iterations}')
                ...
                robotarm7000.servo_point_motion_control(robot=1, end=0, sum=1, count=1, PosVec=[every_joint_angle])
                time.sleep(interval_time)

        # trajectory.append(current_angles[:])       
        # 下面方式不行，因为如果轨迹多了，会超过json数据限制, 不过可以考虑切片分部下发， 后续开发      
        # print(trajectory)
        # sum_frames = len(trajectory)
        # for count in range(1, sum_frames + 1):
        #     end = 1 if count == sum_frames else 0
        #     # 打印count对于的角度
        #     print(f"正在传输第 {count} 帧角度：{trajectory[count - 1]}")
        #     print(f'end={end},sum_frames={sum_frames}, count={count}')
        #     robotarm7000.servo_point_motion_control(robot=1, end=end, sum=sum_frames, count=count, PosVec=trajectory)
        #     time.sleep(interval_time)
        self.logger.info("机械臂已到达目标点")
        return True
    
    def robot_queue_move(self, robotarm6001, trajectory, speed=35, acc=70, dec=70, pl=4):
        """
        队列运动控制
        ...
        if joint_data_type == "List[float]":
            ...
            if reached:
                ...
                monitor_thread = threading.Thread(target=self._monitor_movement, 
                                                  args=(robotarm6001, trajectory, 0, timeout))
                monitor_thread.start()
                monitor_thread.join()
            ...
            monitor_thread = threading.Thread(target=self._monitor_movement, 
                                              args=(robotarm6001, trajectory[-1], 0, timeout))
            monitor_thread.start()
            monitor_thread.join()
    ...
    def robot_disable(self, robotarm6001, servoNum: int=0):
        """
        机械臂对应关节打开抱闸，下使能
        ...
        Returns:
            bool:
                ...
                - False: 抱闸开启失败
        """
        self.logger.info(f'机械臂松开抱闸(robot_disable)')
        if servoNum == 0:
            for joint in range(1, 8):
                robotarm6001.servo_inside_parm_set(joint, "抱闸手动控制", 1)
                # time.sleep(0.5)
                # _, _, _, _, holding_brake_status = robotarm6001.servo_inside_parm_inqure(joint)
                
                # if holding_brake_status != 1:
                #     self.logger.warning(f'伺服 {joint} 抱闸未打开')
                #     return False
                self.logger.info(f'伺服 {joint} 抱闸已打开')
            return True
        else:
            robotarm6001.servo_inside_parm_set(servoNum, "抱闸手动控制", 1)
            time.sleep(0.5)
            _, _, _, _, holding_brake_status = robotarm6001.servo_inside_parm_inqure(servoNum)
            
            if holding_brake_status != 1:
                self.logger.warning(f'伺服 {servoNum} 抱闸未打开')
                return False
            self.logger.info(f'伺服 {servoNum} 抱闸已打开')
            return True

    def robot_enable(self, robotarm6001, servoNum: int=0): 
        """
        机械臂对应关节关闭抱闸，需要后续上电使能
        ...
        Returns:
            bool:
                ...
                - False: 抱闸关闭失败
        """
        self.logger.info(f'机械臂关闭抱闸(robot_enable)')
        if servoNum == 0:
            for joint in range(1, 8):
                robotarm6001.servo_inside_parm_set(joint, "抱闸手动控制", 0)

            # for joint in range(1, 8):
            #     _, _, _, _, holding_brake_status = robotarm6001.servo_inside_parm_inqure(joint)
            #     if holding_brake_status != 0:
                    
            #         time.sleep(2)
            #         _, _, _, _, holding_brake_status = robotarm6001.servo_inside_parm_inqure(joint)
            #         if holding_brake_status != 0:
            #             self.logger.warning(f'伺服 {joint} 抱闸未关闭')
            #             return False
                self.logger.info(f'伺服 {joint} 抱闸已关闭')
            return True
        else:
            robotarm6001.servo_inside_parm_set(servoNum, "抱闸手动控制", 0)
            time.sleep(0.5)
            _, _, _, _, holding_brake_status = robotarm6001.servo_inside_parm_inqure(servoNum)

            if holding_brake_status != 0:
                self.logger.warning(f'伺服 {servoNum} 抱闸未关闭')
                return False
            self.logger.info(f'伺服 {servoNum} 抱闸已关闭')
            return True
